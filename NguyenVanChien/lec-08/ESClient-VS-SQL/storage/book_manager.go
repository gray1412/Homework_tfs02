...
func (bm *BookManager) AddBook(book *Book) error {
	...
	// fill ID if it isn't empty
	if len(book.ID) == 0 {
		//fmt.Println("empty ID")
		book.ID = rndStringRunes(10)
	}

	_, err := bm.esClient.Index().Index(indexName).BodyJson(book.String()).Id(book.ID).Do(ctx)

	// call to fiush data to disk for search. if no call --> need to wait for 5s to search since inserted
...
func (bm *BookManager) DeleteBook(book *Book) error {
	...
	res, err := bm.esClient.Delete().Index(indexName).Id(book.ID).Do(ctx)

	if res.Shards.Successful > 0 {
		fmt.Println("Document deleted from index: ", book.ID)
	}
	return err

}

// func (bm *BookManager) DeleteAllBook() error {
// 	ctx := context.Background()
// 	if bm.esClient == nil {
// 		fmt.Println("Nil es client")
// 		return errors.New("nil es client")
